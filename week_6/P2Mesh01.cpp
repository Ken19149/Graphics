    ...
    while (facesRead < numFaces && std::getline(file, line)) {
        ...
        if (ss >> numVerticesInFace) {
            if (numVerticesInFace == 3) {
                if (ss >> faces[facesRead].v1 >> faces[facesRead].v2 >> faces[facesRead].v3 >> colors[facesRead].r >> colors[facesRead].g >> colors[facesRead].b) {
                    facesRead++;
                }
            } else if (numVerticesInFace == 4) {
                // Convert quad to two triangles
                int v1, v2, v3, v4, r, g, b;
                if (ss >> v1 >> v2 >> v3 >> v4) {
                    colors[facesRead] = {r, g, b};
...
void display() {
    ...
    // Set camera position
    gluLookAt(3.0, 2.0, 3.0,  // Eye position
    ...
    // glColor3f(0.0f, 1.0f, 1.0f); // default color
    
    glBegin(GL_TRIANGLES);
    int i = 0;
    for (const auto& face : faces) {
        glColor3f(colors[i].r, colors[i].g, colors[i].b); // Colors for different faces
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("OFF Mesh Viewer - Table");
