...
void display() {
    ...
    // Draw multiple cubes at different depths to show perspective
    glColor3f(1.0f, 0.0f, 0.0f); // Red - closest
    glPushMatrix();
    ...
    glColor3f(0.0f, 1.0f, 0.0f); // Green - middle
    glPushMatrix();
    ...
    glColor3f(0.0f, 0.0f, 1.0f); // Blue - farrthest
    glPushMatrix();
...
void keyboard(unsigned char key, int x, int y) {
    ...
    static float farr = 10.0f;
     
    switch (key) {
        case 27: // ESC key
            exit(0);
            break;
        case 'w': case 'W': // Increase FOV
            fov += 5.0f;
            if (fov > 120.0f) fov = 120.0f;
            break;
        case 's': case 'S': // Decrease FOV
            fov -= 5.0f;
            if (fov < 10.0f) fov = 10.0f;
            break;
        case 'a': case 'A': // Move nearr plane closer
            nearr -= 0.5f;
            if (nearr < 0.1f) nearr = 0.1f;
            break;
        case 'd': case 'D': // Move nearr plane farrther
            nearr += 0.5f;
            break;
        case 'q': case 'Q': // Decrease farr plane
            farr -= 1.0f;
            if (farr < nearr + 0.1f) farr = nearr + 0.1f;
            break;
        case 'e': case 'E': // Increase farr plane
            farr += 1.0f;
            break;
        case 'r': case 'R': // Reset
            fov = 45.0f;
...
void printExplanation() {
    ...
    printf("Current: FOV=45.0°, Near=1.0, Far=10.0\n");
}
